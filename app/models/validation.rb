class Validation
  ...
  def self.clean_up(n)
    Mongoid::GridFs::File.where(:uploadDate.gte => n.hours.ago).each {|x| Mongoid::GridFs.delete(x.id) }
    Validation.where(:created_at.gte => n.hours.ago, :csv_id.ne => nil).each do |validation|
